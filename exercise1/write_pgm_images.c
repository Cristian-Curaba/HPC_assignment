...
{ 
    ...
    //fread( *image, 1, size, image_file), where size = xsize*ysize*colordepth
    //memset(image, 255, sizeof(image[0][0]) * xsize * ysize);
    
    //write_pgm_image(image, maxval, xsize, ysize, "example_pgm.pgm");
    
    unsigned char *image;
    int maxval1, xsize1, ysize1;
    read_pgm_image((void **)&image, &maxval1, &xsize1, &ysize1, "example_pgm.pgm");
    unsigned char (*pixels)[xsize] = (unsigned char (*)[xsize]) image;
    printf("xsize is %d \n ysize is %d \n maxval is %d \n", xsize1,ysize1,maxval1);
    
    int i = 0;
    while(i<xsize){
    	printf("%d \n", image[i]);    //row-major order
    	i++;
    }
    
    
    unsigned char new_img [xsize][ysize];
    memset(new_img, 255, sizeof(new_img[0][0]) * xsize * ysize);
    int j = 0;
    while(j<xsize){
    	new_img[j][2] = 0;
    	new_img[j][3] = 0;
    ...
    write_pgm_image(new_img, maxval, xsize, ysize, "example.pgm");
